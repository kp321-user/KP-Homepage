...
def get_postgres_history():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, title, slug, era, period, phase, start_year, date_added, last_modified FROM history_page"
    )
    cols = [desc[0] for desc in cursor.description]
    rows = [dict(zip(cols, row)) for row in cursor.fetchall()]
    conn.close()
...
def sync_table(sqlite_records, postgres_records, sqlite_cur, pg_cur, table, columns):
    ...
    for id in all_ids:
        ...
        if in_sqlite and not in_postgres:
            ...
            s = sqlite_dict[id]
            p = postgres_dict[id]
            s_modified = str(s.get("last_modified") or s.get("date_added") or "")
            p_modified = str(p.get("last_modified") or p.get("date_added") or "")
